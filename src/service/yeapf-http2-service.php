...
abstract class HTTP2Service extends Skeleton
{
    ...
    public function setHandler(
        ...
        $isArrayCallable = is_array($attendant) && count($attendant) === 2;
        if (!is_callable($attendant) && !$isArrayCallable) {
        ...
        foreach ($methods as $method) {
            ...
            if (!empty($absentParameter)) {
                ...
                _trace('***********************************');
                // Optional: throw here if strict checking is needed
            } else {
                $this->handlers[$method][$fnPath] = [
                    'attendant'    => $attendant,
                    'path'         => $path,
                    'inlineParams' => $isArrayCallable ? array_combine($inlineParams[0], $inlineParams[1]) : [],
                ];
                ...
                if (is_callable($attendantConstraints)) {
                    ...
                    if ($this->handlers[$method][$fnPath]['operationId'] == null && $isArrayCallable) {
    ...
    private function configureRequestContext(Request $request): void
    {
        ...
        $parsedUrl        = parse_url($this->getExternalURL());
    ...
    private function buildRequestParams(Request $request, string $method): array
    {
        $params = [];
        ...
        foreach ($request as $key => $value) {
            if (null != $value && in_array($key, $allowedParams, true)) {
                $params[$key] = $value;
            }
        }
        return $params;
    ...
    private function handleRequestCallback(Request $request, Response $response): void
    {
        ...
        $this->configureRequestContext($request);
        ...
        try {
            $method = $request->server['request_method'];
            $params = $this->buildRequestParams($request, $method);
            $headers = $request->header;
                    ...
                    if (is_array($security)) {
                        ...
                        if (!empty($knownSchemes[$security])) {
                            ...
                            if ('http' == $secType) {
                                ...
                                if ('bearer' == $secScheme) {
                                    $bearerFormat = $knownSchemes[$security]['bearerFormat'] ?? 'JWT';
                                }
                            }
                        }
                    }
        ...
        if (!$this->ready) {
            ...
            $server->on(
                ...
                function (Request $request, Response $response) {
                    $this->handleRequestCallback($request, $response);
                }
            );
